...
# -------------------- Heart Disease Prediction --------------------
    ...
    tired = st.radio('Do you feel tired while doing small activities?', ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'])
    short_breath = st.radio('Do you feel breathless often?', ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'])
    sweat = st.radio('Do you sweat more than usual during rest?', ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'])
    dizziness = st.radio('Do you feel dizzy or faint?', ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'])
    heartbeat = st.radio('Do you feel your heartbeat is irregular or very fast?', ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'])

    symptoms = [chest_pain, tired, short_breath, sweat, dizziness, heartbeat]
    weights = [12, 9, 11, 8, 7, 10]
    heart_score = sum(weights[i] * ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'].index(symptoms[i]) for i in range(len(weights)))

    if heart_score >= 75:
        st.error(f"🔴 High Heart Disease Risk: {min(100, heart_score)}%")
    elif heart_score >= 50:
        st.warning(f"🟠 Moderate Risk: {min(100, heart_score)}%")
    elif heart_score >= 25:
        st.info(f"🟡 Mild Risk: {min(100, heart_score)}%")
    else:
        st.success(f"🟢 Low Risk: {min(100, heart_score)}%")
